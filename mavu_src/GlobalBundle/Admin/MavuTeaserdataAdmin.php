...
class MavuTeaserdataAdmin extends Admin
{
    /**
        ...
        if ($this->hasSomeWebspacePermission()) {
            $viewCollection->add(
                $this->viewBuilderFactory
                    ->createPreviewFormViewBuilder('sulu_page.page_edit_form.excerpt', '/mavu_teaserdata') // overwrite default excerpt-tab
                    ->disablePreviewWebspaceChooser()
                    ->setResourceKey(BasePageDocument::RESOURCE_KEY)
                    ->setFormKey('page_mavu_teaserdata')
                    ->setTabTitle('mavu.global.tab_teaserdata')
