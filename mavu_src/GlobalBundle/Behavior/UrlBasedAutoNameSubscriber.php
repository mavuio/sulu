...
namespace Mavu\GlobalBundle\Behavior;

// manfred@mavu.io 2022-04-23
// created as a copy from Sulu\Component\DocumentManager\Subscriber\Behavior\Path\AutoNameSubscriber, check for updates of that file
 ...
 */
class UrlBasedAutoNameSubscriber  implements EventSubscriberInterface
{
    /**
    ...
    private function getName(
        ...
        // manfred@mavu.io 2022-04-23
        // only substantial change to original version, take url as base of name

        $segment = null;

        if (is_object($node) && method_exists($node, "getPropertyValueWithDefault")) {
            $segment = $node->getPropertyValueWithDefault("i18n:de-mavu_seo-fixed_path", null);
        }

        if (!$segment && method_exists($document, 'getResourceSegment')) {
            $segment = basename($document->getResourceSegment());
        }

        if (!$segment) {
            $segment = $document->getTitle();
        }

        if (!$segment) {
            throw new DocumentManagerException(
                \sprintf(
                    'Document has no url-segment (an url-segment is required for auto name behavior): %s)',
        ...
        $name = $this->slugifier->slugify($segment);
